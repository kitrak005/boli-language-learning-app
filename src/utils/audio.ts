...
class SoundEngine {
  ...
  speak(
    ...
    try {
      ...
      console.log('[VAKYA DEBUG] total voices loaded:', this.voices.length);

      // Check available voices that can ACTUALLY read native Indic script (Devanagari/Tamil/etc).
      // IMPORTANT: only match real Indic language codes here. Do NOT match on
      // v.lang.includes('in') or name.includes('india') — those also match
      // "en-IN" (English, India) voices like "Microsoft Ravi", which cannot
      // pronounce Devanagari/Tamil script and will silently mumble/skip it.
      const indianVoice = this.voices.find((v) => {
        ...
        const nativeLangCode =
          ...
          lang.startsWith('sa');   // Sanskrit
        const namedNativeVoice =
          name.includes('hindi') ||
          name.includes('tamil') ||
          name.includes('veena') ||   // known Tamil voice
          name.includes('kalpana') || // known Telugu voice
          name.includes('lekha');     // known Hindi voice
        return nativeLangCode || namedNativeVoice;
      });
      ...
      utterance.onstart = () => {
        console.log('[VAKYA DEBUG] utterance onstart fired');
      ...
      utterance.onerror = (e) => {
        console.warn('[VAKYA DEBUG] SpeechSynthesis utterance error, running acoustic fallback', e);
      ...
      // IMPORTANT FIX: calling speak() in the same tick as cancel() is a known
      // Chrome/Edge race condition that can silently drop the utterance.
      // Give the browser a moment to finish processing cancel() first.
      setTimeout(() => {
        window.speechSynthesis.speak(utterance);
        console.log(
          '[VAKYA DEBUG] speak() called | pending:',
      ...
      console.warn('[VAKYA DEBUG] SpeechSynthesis exception, fallback to acoustic resonance', err);
