...
// serverless function (Vercel keeps the module scope alive between requests
// on a warm instance, same benefit as the singleton pattern in server.ts).
...
export default async function handler(req: any, res: any) {
    ...
    try {
        ...
        if (!query || typeof query !== 'string' || !query.trim()) {
            res.status(400).json({ error: 'Please provide a word or term to ask the Guru.' });
            return;
        }
